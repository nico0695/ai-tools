...
| `ai_setups` | result of `sddl-init` steps 4-7: `detected`, `configured` (`claude_code` / `agents`), `skills_installed` (one entry per install target: target + method + timestamp + skills), `wrappers_injected` |
