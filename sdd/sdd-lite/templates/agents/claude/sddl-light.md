...
description: >
  ...
  sddl-light for sddl-proposal, sddl-archive, or sddl-delivery.
  Do not use for spec, design, code execution, deep exploration, or review.
model: haiku
effort: low
...
- First: Read `<package-root>/skills/<stage>/SKILL.md` in full for the skill named in `stage`. Follow only that skill and return exactly the fields in its "Expected Output" section.
