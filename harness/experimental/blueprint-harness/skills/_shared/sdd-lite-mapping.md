...
Correspondence between `blueprint-harness` (`bp-*`) and `sdd-lite` (`sddl-*`) vocabulary. Contracts are blueprint-owned but mirror sdd-lite deliberately (analysis decision D9); this doc exists to keep the mirroring conscious. Physical sharing (`harness/_shared/`) is deferred until both stabilize.
